...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box}html,body{max-width:100%;overflow-x:hidden}img,canvas,audio,video{max-width:100%}.grid{grid-template-columns:repeat(auto-fit,minmax(min(100%,280px),1fr))}.grid>*,.panel>*,.card>*,.row>*,.identity>*,.nav-group,.nav-links{min-width:0}input,select,textarea{width:100%;max-width:100%}input[type=checkbox],input[type=radio],input[type=range]{width:auto}.row strong,.row p,.identity strong,.muted{overflow-wrap:anywhere}.nav-toggle{display:none;align-items:center;justify-content:center;width:44px;height:44px;padding:0;background:#fff;color:#344054;border:1px solid #d0d5dd;box-shadow:0 1px 2px rgba(16,24,40,.05)}.nav-toggle:hover{background:#f9fafb;color:#101828}.nav-toggle svg{width:23px;height:23px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round}.nav-close{display:none}.nav-backdrop{display:none}.mobile-nav-actions{display:none}
    @media(max-width:760px){.shell{padding:16px 14px 48px}.app-header{align-items:center;margin-bottom:14px}.app-header .muted{display:none}.app-header>.quiet-action{display:none}.app-header>.row{display:flex;align-items:center}.nav-toggle{display:inline-flex;flex:0 0 44px}.app-nav{position:fixed;z-index:90;top:0;right:0;bottom:0;width:min(86vw,340px);margin:0;padding:76px 18px 24px;display:block;overflow-y:auto;border:0;border-radius:0;background:#fff;box-shadow:-18px 0 50px rgba(16,24,40,.18);transform:translateX(105%);visibility:hidden;transition:transform .22s ease,visibility .22s}.app-nav.is-open{transform:translateX(0);visibility:visible}.nav-group{display:grid;width:100%;gap:7px;margin-bottom:20px}.nav-label{padding:0 10px;color:#667085;font-size:11px}.nav-links,.child-app-nav .nav-links{display:grid;justify-content:stretch;gap:4px}.app-nav a{width:100%;min-height:44px;padding:10px 12px;font-size:16px}.app-nav a[aria-current=page]{background:var(--primary-soft)}.nav-close{display:inline-flex;position:absolute;top:16px;right:18px}.mobile-nav-actions{display:grid;padding-top:16px;margin-top:8px;border-top:1px solid var(--line)}.mobile-nav-actions .btn{text-align:center}.nav-backdrop{position:fixed;z-index:80;inset:0;display:block;background:rgba(16,24,40,.55);opacity:0;visibility:hidden;transition:opacity .22s,visibility .22s;backdrop-filter:blur(2px)}.nav-backdrop.is-open{opacity:1;visibility:visible}body.nav-open{overflow:hidden;touch-action:none}.panel{padding:17px}.grid{gap:14px}.top{gap:10px}.points.big{font-size:18px;padding:8px 10px}.quiet-action{padding:8px 10px}.assign{grid-template-columns:minmax(0,1fr) auto}.preview-banner{align-items:flex-start;gap:10px}.preview-banner span{min-width:0}.calendar-actions h2{font-size:17px;text-align:center}.calendar-actions .btn{padding:8px;font-size:0}.calendar-actions .btn:first-child:after{content:'←';font-size:16px}.calendar-actions .btn:last-child:before{content:'→';font-size:16px}}
    @media(max-width:440px){.row{align-items:stretch}.card>.row,.preview-banner{display:grid}.card>.row>.btn,.preview-banner>.btn{width:100%;text-align:center}.assign{grid-template-columns:1fr}.assign button{width:100%}.identity{min-width:0}.identity .avatar{flex:0 0 auto}.message-bubble{max-width:90%}.reward-card{width:100%}.calendar-day{min-width:0}.calendar-count{overflow:hidden;text-overflow:ellipsis}.top .row{display:flex;align-items:center}.top .row .btn{width:auto}}
    @media(prefers-reduced-motion:reduce){.app-nav,.nav-backdrop{transition:none}}
  </style>
...
(() => {
  ...
  const opener = document.querySelector('.app-header .nav-toggle');
  const nav = opener && document.getElementById(opener.getAttribute('aria-controls'));
  const backdrop = document.querySelector('.nav-backdrop');
  if (opener && nav && backdrop) {
    const closer = nav.querySelector('.nav-close');
    ...
    const openMenu = () => {
      ...
      requestAnimationFrame(() => (closer || focusable()[0]).focus());
    };
    opener.addEventListener('click', () => opener.getAttribute('aria-expanded') === 'true' ? closeMenu() : openMenu());
    ...
    desktop.addEventListener('change', event => { if (event.matches) closeMenu(false); });
  }
...
{{define "admin_top"}}
<header class="top app-header"><div class="identity">{{if .ParentImage}}<img class="avatar" src="{{.ParentImage}}" alt="Parent profile">{{else}}<span class="avatar avatar-placeholder">P</span>{{end}}<div><div class="brand">iparent</div><div class="muted">Parent workspace</div></div></div><a class="btn secondary quiet-action" href="/logout">Log out</a><button class="nav-toggle" type="button" aria-label="Open menu" aria-expanded="false" aria-controls="parent-nav"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg></button></header>
<nav class="app-nav" id="parent-nav" aria-label="Parent portal"><button class="nav-toggle nav-close" type="button" aria-label="Close menu"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button><div class="nav-group"><span class="nav-label">Family</span><div class="nav-links"><a href="/admin/children">Children</a><a href="/admin/messages">Messages</a><a href="/admin/music">Music</a></div></div><div class="nav-group"><span class="nav-label">Activities</span><div class="nav-links"><a href="/admin/challenges">Challenges</a><a href="/admin/schedules">Schedule</a><a href="/admin/review">Review{{if .Pending}}<span class="nav-badge">{{len .Pending}}</span>{{end}}</a></div></div><div class="nav-group"><span class="nav-label">Motivation</span><div class="nav-links"><a href="/admin/rewards">Rewards</a><a href="/admin/points">Points</a><a href="/admin/strikes">Strikes</a><a href="/admin/history">History</a></div></div><div class="mobile-nav-actions"><a class="btn secondary" href="/logout">Log out</a></div></nav><div class="nav-backdrop" aria-hidden="true"></div>
{{if .Error}}<div class="error">{{.Error}}</div>{{end}}
...
<div class="top app-header"><div class="identity">{{if .Child.ProfileImage}}<img class="avatar" src="{{.Child.ProfileImage}}" alt="Profile">{{else}}<span class="avatar avatar-placeholder">C</span>{{end}}<div><div class="brand">Hi, {{if .Child.Name}}{{.Child.Name}}{{else}}{{.Child.Username}}{{end}}</div><div class="muted">{{.Child.Done}} of {{.Child.Total}} assigned challenges complete</div></div></div><div class="row"><span class="pill points big">{{.Child.Points}} pts</span>{{if not .Preview}}<a class="btn secondary quiet-action" href="/logout">Log out</a><button class="nav-toggle" type="button" aria-label="Open menu" aria-expanded="false" aria-controls="child-nav"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg></button>{{end}}</div></div>
{{if not .Preview}}<nav class="app-nav child-app-nav" id="child-nav" aria-label="Child portal"><button class="nav-toggle nav-close" type="button" aria-label="Close menu"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button><div class="nav-group"><span class="nav-label">My space</span><div class="nav-links"><a href="/child">Challenges</a><a href="/child/completed">Completed</a><a href="/child/rewards">Rewards</a><a href="/child/messages">Messages</a><a href="/child/music">Music</a><a href="/child/history">History</a></div></div><div class="mobile-nav-actions"><a class="btn secondary" href="/logout">Log out</a></div></nav><div class="nav-backdrop" aria-hidden="true"></div>{{end}}
